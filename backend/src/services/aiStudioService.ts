...
const normalizeFlashcardDeckPayload = (
  ...
  const cleanCardText = (value: string) =>
    clip(value, 3000)
      ...
      .replace(/\n{3,}/g, '\n\n')
      .trim();
  const combineBack = (back: unknown, explanation: unknown) => {
    const answer = cleanCardText(String(back || ''));
    const detail = cleanCardText(String(explanation || ''));
    if (!detail || answer.includes(detail)) return answer;
    return [answer, detail].filter(Boolean).join('\n\n');
  };
  const normalizedFallbackTitle = fallbackTitle.replace(/\s+/g, '').trim();
  const looksLikeRawRequest = (front: string) => {
    const normalized = front.replace(/\s+/g, '').trim();
    return Boolean(
      normalized &&
        ((normalizedFallbackTitle && normalized === normalizedFallbackTitle) ||
          /^(请|给我|帮我|生成|制作|创建|把).{0,24}(flashcards?|复习卡|抽认卡|卡片|card)/i.test(front))
    );
  };
  const isValidFront = (front: string) => {
    const text = front.replace(/\s+/g, ' ').trim();
    if (!text || looksLikeRawRequest(text)) return false;
  ...
  return {
    ...
    cards: cards
      .map((card, index): StructuredFlashcardInput | null => {
        ...
        const back = combineBack(card?.back, card?.explanation);
        if (!front || !back || !isValidFront(front)) return null;
...
const buildFallbackFlashcardDeck = (input: StudioGenerateInput, capsule: ContextCapsule): StudioFlashcardDeckPayload => {
  ...
  const cards = (evidence.length ? evidence : [topic]).map((item, index) => ({
    front: evidence.length
      ? index === 0
        ? '这个来源材料中的核心概念是什么？'
        : '这个来源片段支持了哪个关键知识点？'
      : '当前证据不足时，生成复习卡前需要先补充什么资料？',
    back: item,
...
const buildFallbackContent = (input: StudioGenerateInput, capsule: ContextCapsule) => {
  ...
  const hasSources = capsule.citations.length > 0 || evidence.length > 0;
  ...
  if (input.resourceType === 'data_table') {
    const rows = [
      ...
      [
        ...
        hasSources ? 'Based on the selected sources' : 'Based on model knowledge because no sources were selected',
        (evidence[0] || 'No selected source evidence available').replace(/"/g, '""'),
        hasSources ? 'Add a concrete example after reviewing the source' : 'Add a concrete example from general knowledge',
        hasSources ? 'Do not overgeneralize beyond the available sources' : 'Verify against course sources before relying on it',
  ...
  if (input.resourceType === 'mind_map') {
    ...
    return [
      ...
      '',
      '## 大纲',
      evidence.map((item, index) => `${index + 1}. ${item}`).join('\n') || '- 未选择来源；以下结构基于模型通用知识生成。',
      '',
      '## 来源说明',
      sourceNotes || '- 未选择来源；以上内容基于模型通用知识，建议补充 sources/files 后重新生成。'
    ].join('\n');
  }
  ...
  return [
    ...
    evidence.map((item, index) => `### 要点 ${index + 1}\n${item}`).join('\n\n') ||
      '未选择来源；以下内容基于模型通用知识生成，建议补充 sources/files 后重新生成。',
    '',
    '## 来源',
    citationList(capsule) || '- 未选择来源',
    '',
    '## 下一步',
    '- 检查来源是否覆盖目标范围。',
        ...
        const normalized: StudioQuizQuestion = {
          ...
          choiceFeedback:
            question.choiceFeedback && typeof question.choiceFeedback === 'object'
              ? {
                  A: question.choiceFeedback.A ? clip(question.choiceFeedback.A, 40) : undefined,
                  B: question.choiceFeedback.B ? clip(question.choiceFeedback.B, 40) : undefined,
                  C: question.choiceFeedback.C ? clip(question.choiceFeedback.C, 40) : undefined,
                  D: question.choiceFeedback.D ? clip(question.choiceFeedback.D, 40) : undefined
                }
              : undefined,
...
const buildGenerationPrompt = (input: StudioGenerateInput, capsule: ContextCapsule, learnerContext?: LearnerStateAgentContext) => {
  ...
  const groundingRule = capsule.citations.length
    ? '你必须严格基于 Context Capsule 中的证据生成内容；依据不足时要说明不足，不要编造。'
    : '用户没有选择 sources/files；请不要声称基于当前 Workbench 或当前文件，只能基于用户要求和你的通用知识回答，并在需要来源支撑处说明未提供来源。';
  return [
...
const normalizedQuizPayload = (input: StudioGenerateInput, capsule: ContextCapsule, payload: StudioQuizPayload) => {
  const contract = parsePracticeRequestContract({
    prompt: input.prompt,
    options: input.options || {},
    templateId: undefined,
    defaultOptions: {
      questionCount: Number(input.options?.questionCount || 3),
      difficulty: String(input.options?.difficulty || 'adaptive')
    }
  });
  const topic = plainQuizTopic(input, capsule);
  return reconcileQuizWithPracticeContract(payload, contract, topic);
};
...
export class AiStudioService {
  async generate(input: StudioGenerateInput) {
    ...
    try {
      ...
      if (input.resourceType === 'quiz' && aiModelProviderService.isConfigured({ useCase: 'studio' })) {
        try {
          const response = await aiModelProviderService.json<StudioQuizPayload>({
            useCase: 'studio',
            instruction: [
              '你是 AI Studio 的 Quiz Generator。',
              capsule.citations.length
                ? '请严格基于 Context Capsule 生成可交互测验。'
                : '用户未选择 sources/files；请基于用户要求和你的通用知识生成测验，不要声称题目来自当前 Workbench、当前文件、选区或视口。',
              ...
              '每题尽量包含一个简短的 hint。选择题额外提供 choiceFeedback，对 A/B/C/D 每个选项只写一句很短的选项层反馈，控制在十几个字内；完整解析只放 explanation。',
            ...
            input: {
              ...
              groundingMode: capsule.citations.length ? 'selected_sources' : 'model_knowledge_no_sources_selected',
              ...
              contextPreview: capsule.promptContextPreview || (capsule.citations.length ? '' : '未选择来源；不要使用当前 Workbench、当前文件、选区或视口作为依据。'),
            ...
            context: { visualEvidence: capsule.visualEvidence },
            timeoutMs: STUDIO_MODEL_TIMEOUT_MS
          });
          const normalizedQuiz = normalizedQuizPayload(input, capsule, normalizeQuizPayload(response.data, latestUserPrompt(input)));
          ...
          const fallbackQuiz = normalizedQuizPayload(input, capsule, normalizeQuizPayload(JSON.parse(buildFallbackContent(input, capsule)), latestUserPrompt(input)));
        ...
        content = JSON.stringify(reviewed.quiz, null, 2);
        qualityReport = reviewed.report;
      } else if (input.resourceType === 'flashcards' && aiModelProviderService.isConfigured({ useCase: 'studio' })) {
        try {
          const response = await aiModelProviderService.json<StudioFlashcardDeckPayload>({
            useCase: 'studio',
            instruction: [
              '你是 AI Studio 的 source-grounded Flashcard Generator。',
              capsule.citations.length
                ? '严格基于 Context Capsule 生成结构化卡组。'
                : '用户未选择 sources/files；请基于用户要求和你的通用知识生成结构化卡组，不要声称卡片来自当前 Workbench、当前文件、选区或视口。',
              '每张卡必须短 front、清晰 back、卡级 sourceRefs、difficulty、concept、explanation。',
              '优先生成可主动回忆的卡片，不要把整段资料原文作为题面。',
              'front 必须是问题或 cloze，不要写“速记卡片”“Card 1”“基础卡片”这类标题。',
              'back 自然合并答案和必要解释，不要输出 Difficulty:、Explanation:、Source: 这类字段标签。',
        ...
        flashcardDeckPayload = buildFallbackFlashcardDeck(input, capsule);
        content = flashcardDeckToMarkdown(flashcardDeckPayload);
      } else if (aiModelProviderService.isConfigured({ useCase: 'studio' })) {
        try {
          const response = await aiModelProviderService.chat(
            [{ role: 'user', content: buildGenerationPrompt(input, capsule, learnerContext) }],
            undefined,
            { timeoutMs: STUDIO_MODEL_TIMEOUT_MS, useCase: 'studio', visualEvidence: capsule.visualEvidence }
          );
      ...
      const file = await FileSystemService.saveGeneratedContent({
        ...
        origin: 'ai',
        metadata: { generator: 'studio', artifactType: input.resourceType },
        content
      });
      const flashcardDeck = flashcardDeckPayload
        ? await flashcardService.createDeck({
            ...
            source: source.startsWith('fallback') ? 'ai_studio_fallback' : `ai_studio_${source}`,
  ...
  async judgeQuizAnswer(input: JudgeQuizAnswerInput) {
    ...
    if (!aiModelProviderService.isConfigured({ useCase: 'studio' })) {
      const result = {
        correct: false,
        score: 0,
        feedback: '后端 AI judge 未配置可用模型服务，暂时无法提供解释型评分反馈。',
      ...
      await learningEventCollectionService.collect({
        ...
        actor: 'user',
        object: { type: 'quiz_question', id: question.id, title: question.question },
        interaction: { userText: userAnswer, answerCorrect: result.correct, score: result.score },
        payload: { question, result },
        confidence: 0.72
      }).catch((error) => console.warn('Learning event collection fallback quiz failed:', error));
      await courseKnowledgeGraphService.bindQuizQuestion({
        workspaceId: input.workspaceId,
        workbenchId: input.workbenchId || null,
        question,
        result
      }).catch((error) => console.warn('Course KG fallback quiz binding failed:', error));
      return result;
    }
    ...
    const response = await aiModelProviderService.json<{
      ...
      instruction: [
        ...
        '选择题也必须结合题干、选项和标准答案给出解释性反馈；选项下方的短反馈只指出哪里对或哪里错，完整解析放在 feedback。',
        '填空题允许同义词、近似表达、大小写和轻微书写差异。',
        '解答题按语义评分，不要求逐字一致；可以给部分分。',
        'feedback 用简体中文，必须像老师批改一样解释，不要只报分数和正确答案。',
        '不要套用“表面信息”“常见误区”等固定话术；只有题目证据明确支持时才指出具体误区。',
        'score 必须是 0 到 1 的数字。feedback 具体指出命中点、缺失点、选项差异、依据或下一步建议。',
        '只输出 JSON。'
      ].join('\n'),
      schema: {
        correct: 'boolean',
    ...
    await learningEventCollectionService.collect({
      ...
      payload: { question, result },
      confidence: 0.82
    }).catch((error) => console.warn('Learning event collection quiz failed:', error));
    await courseKnowledgeGraphService.bindQuizQuestion({
      workspaceId: input.workspaceId,
      workbenchId: input.workbenchId || null,
      question,
      result
    }).catch((error) => console.warn('Course KG quiz binding failed:', error));
    return result;
  }
  ...
  async assistQuizQuestion(input: QuizQuestionAssistantInput) {
    if (!aiModelProviderService.isConfigured({ useCase: 'studio' })) {
      await learningEventCollectionService.collect({
        ...
        eventType: 'hint.used',
        actor: 'user',
        object: { type: 'quiz_question', id: input.question.id, title: input.question.question },
        interaction: { hintUsed: true },
        payload: { unavailable: true, question: input.question },
        confidence: 0.6
      }).catch((error) => console.warn('Learning event collection fallback hint failed:', error));
      return {
        reply: '后端 AI 助手暂未配置可用模型服务，当前无法解释这道题。',
    ...
    await learningEventCollectionService.collect({
      ...
      interaction: {
        ...
        assistantText: response.data.reply || ''
      },
      payload: {
        question: input.question,
        userAnswer: input.userAnswer || '',
      ...
      confidence: 0.68
    }).catch((error) => console.warn('Learning event collection hint failed:', error));
