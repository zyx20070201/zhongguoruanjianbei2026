 ...
 */
async function importHTMLToDoc({ collection, html, fileName, }) {
    const job = new Job({
        collection,
        middlewares: [
 ...
 */
async function importHTMLZip({ collection, imported }) {
    ...
    await Promise.all(htmlBlobs.map(async ([fileName, blob]) => {
