...
export const replaceIdMiddleware = ({ slots, collection }) => {
    ...
    slots.afterImport.on(payload => {
        ...
        if (payload.type === 'block' &&
            ['affine:embed-linked-doc', 'affine:embed-synced-doc'].includes(payload.snapshot.flavour)) {
            const model = payload.model;
...
const customDocLinkBaseUrlMiddleware = (baseUrl) => {
...
export const docLinkBaseURLMiddlewareBuilder = (baseUrl) => {
...
const defaultDocLinkBaseURLMiddlewareBuilder = docLinkBaseURLMiddlewareBuilder(typeof window !== 'undefined' ? window.location.origin : '.');
export const docLinkBaseURLMiddleware = defaultDocLinkBaseURLMiddlewareBuilder.get();
export const setDocLinkBaseURLMiddleware = defaultDocLinkBaseURLMiddlewareBuilder.set;
