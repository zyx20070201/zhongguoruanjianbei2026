...
import { defaultImageProxyMiddleware, docLinkBaseURLMiddleware, fileNameMiddleware, titleMiddleware, } from './middlewares.js';
 ...
 */
async function importMarkdownToBlock({ doc, markdown, blockId, }) {
    const job = new Job({
        collection: doc.collection,
 ...
 */
async function importMarkdownToDoc({ collection, markdown, fileName, }) {
    const job = new Job({
        collection,
        middlewares: [
 ...
 */
async function importMarkdownZip({ collection, imported, }) {
    ...
    await Promise.all(markdownBlobs.map(async ([fileName, blob]) => {
