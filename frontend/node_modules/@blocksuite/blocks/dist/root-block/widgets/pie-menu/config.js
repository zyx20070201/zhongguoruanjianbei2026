...
pie.command({
    label: 'Note',
    icon: NoteIcon,
    action: setEdgelessToolAction(tool => tool.setTool('affine:note', {
...
pie.command({
    ...
    action: setEdgelessToolAction(tool => tool.setTool('connector', {
...
shapes.forEach(shape => {
    pie.command({
        label: shape.label,
        icon: ({ rootComponent }) => {
            const attributes = rootComponent.std.get(EditPropsStore).lastProps$.value[`shape:${shape.type}`];
...
pie.command({
    label: 'Toggle Style',
    icon: ({ rootComponent }) => {
        const { shapeStyle } = rootComponent.std.get(EditPropsStore).lastProps$.value['shape:roundedRect'];
