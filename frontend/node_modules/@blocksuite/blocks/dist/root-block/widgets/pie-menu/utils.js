...
export function getActiveShapeColor(type) {
    return ({ rootComponent }) => {
        if (rootComponent instanceof EdgelessRootBlockComponent) {
            const props = rootComponent.std.get(EditPropsStore).lastProps$.value['shape:roundedRect'];
...
export function getActiveConnectorStrokeColor({ rootComponent, }) {
    if (rootComponent instanceof EdgelessRootBlockComponent) {
        const props = rootComponent.std.get(EditPropsStore).lastProps$.value.connector;
        const color = props.stroke;
        return color.toString();
    }
    return '';
}
export function setEdgelessToolAction(callback) {
    return ({ rootComponent }) => {
