    ...
    toolbar
        ...
        action: (chain, formatBar) => {
            ...
            void promptDocTitle(host, autofill).then(async (title) => {
                if (title === null)
                    return;
                await convertSelectedBlocksToLinkedDoc(host.std, doc, draftedModels, title);
                ...
                action: c => {
                    ...
                    c.std.command
                        ...
                        ctx.draftedModels
                            ...
                            return ctx.std.clipboard.duplicateSlice(slice, ctx.std.doc, ctx.parentBlock?.model.id, ctx.blockIndex ? ctx.blockIndex + 1 : 1);
                        })
                            .catch(console.error);
                        return next();
                    })
