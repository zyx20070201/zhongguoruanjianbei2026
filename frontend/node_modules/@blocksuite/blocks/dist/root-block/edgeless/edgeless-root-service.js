...
import { MindmapElementModel, RootBlockSchema, } from '@blocksuite/affine-model';
import { EditPropsStore } from '@blocksuite/affine-shared/services';
import { clamp } from '@blocksuite/affine-shared/utils';
import { GfxControllerIdentifier, GfxExtensionIdentifier, isGfxGroupCompatibleModel, } from '@blocksuite/block-std/gfx';
...
export class EdgelessRootService extends RootService {
    ...
    createTemplateJob(type, center) {
        const middlewares = [];
        if (type === 'template') {
            const bounds = [...this.blocks, ...this.elements].map(i => Bound.deserialize(i.xywh));
        ...
        if (type === 'sticker') {
            middlewares.push(createStickerMiddleware(center || this.viewport.center, () => this.layer.generateIndex()));
        }
    ...
    getFitToScreenData(padding = [0, 0, 0, 0], inputBounds) {
        ...
        return this.viewport.getFitToScreenData(bound, padding, ZOOM_INITIAL, FIT_TO_SCREEN_PADDING);
    }
    mounted() {
        super.mounted();
