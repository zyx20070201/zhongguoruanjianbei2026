...
export class EdgelessSnapManager extends Overlay {
    ...
    get _surface() {
        const surfaceModel = this.gfx.doc.getBlockByFlavour('affine:surface')[0];
        return this.gfx.std.view.getBlock(surfaceModel.id);
    }
    constructor(gfx) {
        super(gfx);
    ...
    setupAlignables(alignables, exclude = []) {
        if (alignables.length === 0)
            return new Bound();
        const connectors = alignables.filter(isConnectable).reduce((prev, el) => {
            const connectors = this.gfx.surface.getConnectors(el.id);
