...
import { handleNativeRangeAtPoint, humanFileSize, } from '@blocksuite/affine-shared/utils';
import { GfxControllerIdentifier } from '@blocksuite/block-std/gfx';
import { Bound, Point, serializeXYWH, Vec, } from '@blocksuite/global/utils';
import { getFileType, uploadAttachmentBlob, } from '../../../attachment-block/utils.js';
...
export async function addAttachments(std, files, point) {
    if (!files.length)
        return [];
    ...
    if (isSizeExceeded) {
        toast(std.host, `You can only upload files less than ${humanFileSize(maxFileSize, true, 0)}`);
        return [];
    }
    ...
    const dropInfos = files.map((file, index) => {
        ...
        const blockId = std.doc.addBlock('affine:attachment', {
            name: file.name,
            size: file.size,
            type: file.type,
            style: 'cubeThick',
...
export async function addImages(std, files, point) {
    ...
    imageFiles.map((file, index) => {
        ...
        const blockId = std.doc.addBlock('affine:image', {
            size: file.size,
            xywh: bound.serialize(),
        }, gfx.surface);
...
export function addNoteAtPoint(std, 
/**
 * The point is in browser coordinate
 */
point, options = {}) {
    const gfx = std.get(GfxControllerIdentifier);
    const { width = DEFAULT_NOTE_WIDTH, height = DEFAULT_NOTE_HEIGHT, offsetX = DEFAULT_NOTE_OFFSET_X, offsetY = DEFAULT_NOTE_OFFSET_Y, parentId = gfx.doc.root?.id, noteIndex: noteIndex, scale = 1, } = options;
    const [x, y] = gfx.viewport.toModelCoord(point.x, point.y);
    const blockId = addBlock(std, 'affine:note', {
...
export function addNote(std, point, options, width = DEFAULT_NOTE_WIDTH, height = DEFAULT_NOTE_HEIGHT) {
