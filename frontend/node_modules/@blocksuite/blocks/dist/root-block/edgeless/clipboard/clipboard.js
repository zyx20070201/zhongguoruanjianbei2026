import { CanvasElementType, SurfaceGroupLikeModel, TextUtils, } from '@blocksuite/affine-block-surface';
import { BookmarkStyles, DEFAULT_NOTE_HEIGHT, DEFAULT_NOTE_WIDTH, ReferenceInfoSchema, } from '@blocksuite/affine-model';
import { EmbedOptionProvider, ParseDocUrlProvider, TelemetryProvider, } from '@blocksuite/affine-shared/services';
import { isInsidePageEditor, isUrlInClipboard, matchFlavours, referenceToNode, } from '@blocksuite/affine-shared/utils';
import { compareLayer, SortOrder, } from '@blocksuite/block-std/gfx';
...
export class EdgelessClipboardController extends PageClipboard {
    ...
    _createCanvasElement(clipboardData, context, newXYWH) {
    ...
    _createFrameBlock(frame, context) {
    ...
    async createElementsFromClipboardData(elementsRawData, pasteCenter) {
        ...
        const context = {
            ...
            newPresentationIndexes: createNewPresentationIndexes(elementsRawData, this.edgeless),
        };
        const blockModels = [];
        ...
        for (const data of elementsRawData) {
            ...
            if (blockSnapshot) {
                ...
                assertType(data);
                const oldId = data.id;
                const element = this._createCanvasElement(data, context, getNewXYWH(data.xywh));
