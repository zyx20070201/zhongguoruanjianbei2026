...
import { generateKeyBetweenV2, getTopElements, GfxExtension, GfxExtensionIdentifier, isGfxGroupCompatibleModel, renderableInEdgeless, } from '@blocksuite/block-std/gfx';
...
export class FrameOverlay extends Overlay {
    ...
    get _frameManager() {
        return this.gfx.std.get(GfxExtensionIdentifier('frame-manager'));
    }
    constructor(gfx) {
        super(gfx);
...
export class EdgelessFrameManager extends GfxExtension {
    ...
    static framePresentationComparator(a, b) {
        ...
        if ('presentationIndex$' in a &&
            'presentationIndex$' in b &&
            a.presentationIndex$.value &&
            b.presentationIndex$.value) {
            return stringCompare(a.presentationIndex$.value, b.presentationIndex$.value);
        }
    ...
    _addFrameBlock(bound) {
        ...
        const id = this.gfx.doc.addBlock('affine:frame', {
            title: new Text(new DocCollection.Y.Text(`Frame ${this.frames.length + 1}`)),
    ...
    _watchElementAdded() {
        ...
        this._disposable.add(surfaceModel.elementAdded.on(({ id, local }) => {
    ...
    generatePresentationIndex() {
        const before = this.frames[this.frames.length - 1]?.presentationIndex ?? null;
    ...
    refreshLegacyFrameOrder() {
        ...
        let afterPreIndex = frames[splitIndex]?.presentationIndex || generateKeyBetweenV2(null, null);
...
export function getBlocksInFrameBound(doc, model, fullyContained = true) {
    ...
    if (!surface)
        return [];
    return getNotesInFrameBound(doc, model, fullyContained).concat(surface.children.filter(ele => {
        if (ele.id === model.id)
            return;
