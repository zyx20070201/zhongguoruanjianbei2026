...
import { BlockNotionHtmlAdapterExtension, HastUtils, TextUtils, } from '@blocksuite/affine-shared/adapters';
                            ...
                            if (HastUtils.querySelector(child, '.selected-value')) {
                                ...
                                child.type === 'element' &&
                                    child.children.forEach(span => {
                                        const filteredArray = columns[index].data.options?.filter(option => option.value === HastUtils.getTextContent(span));
