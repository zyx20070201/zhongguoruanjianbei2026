import { ParagraphBlockModel, ParagraphBlockSchema, } from '@blocksuite/affine-model';
import { DragHandleConfigExtension } from '@blocksuite/affine-shared/services';
import { calculateCollapsedSiblings, captureEventTarget, matchFlavours, } from '@blocksuite/affine-shared/utils';
export const ParagraphDragHandleOption = DragHandleConfigExtension({
    ...
    onDragStart: ({ state, startDragging, anchorBlockId, editorHost }) => {
        ...
        if (matchFlavours(model, ['affine:paragraph']) &&
            ...
            model.collapsed) {
            const collapsedSiblings = calculateCollapsedSiblings(model).flatMap(sibling => editorHost.view.getBlock(sibling.id) ?? []);
    ...
    onDragEnd: ({ draggingElements }) => {
        draggingElements
            ...
            if (!(model instanceof ParagraphBlockModel))
                return;
...
//# sourceMappingURL=paragraph-drag-extension.js.map