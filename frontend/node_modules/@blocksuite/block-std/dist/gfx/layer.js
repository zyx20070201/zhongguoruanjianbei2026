import { assertType, Bound, DisposableGroup, last, Slot, } from '@blocksuite/global/utils';
...
export class LayerManager {
    ...
    watch(blocks) {
        ...
        if (doc) {
            this._disposable.add(doc.slots.blockUpdated.on(payload => {
                ...
                if (payload.type === 'update') {
                    ...
                    else if (this.blocks.includes(block) &&
                        !(block.parent instanceof SurfaceBlockModel ||
                            block.parent?.role === 'root')) {
                        this.delete(block);
                    }
        ...
        if (surface) {
            ...
            this._disposable.add(surface.localElementAdded.on(elm => {
                this.add(elm);
            }));
            this._disposable.add(this._surface.localElementUpdated.on(payload => {
                if (payload.props['index'] || payload.props['groupId']) {
                    this.update(payload.model, payload.props);
                }
            }));
            this._disposable.add(surface.localElementDeleted.on(elm => {
                this.delete(elm);
            }));
